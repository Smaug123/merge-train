...
proptest! {
    ...
    fn squash_has_single_parent(
        ...
        let squash = squash_merge_to_main(&config, &branch_sha);
        ...
        let parents = super::get_parents(&worktree, squash.squash_sha.as_str()).unwrap();
...
fn squash_parent_ordering_incorporates_late_commits() {
    ...
    let parents = super::get_parents(&clone_dir, squash.squash_sha.as_str()).unwrap();
    assert_eq!(
        parents.len(),
        1,
    ...
    assert_eq!(
        parents[0], squash.prior_main_head,
...
/// Property 8: Fan-out worktree ordering (direct removal).
///
...
fn recovery_uses_frozen_descendants() {
    ...
    let squash_is_ancestor =
        is_ancestor(&worktree2, &squash.squash_sha, &head_sha).unwrap_or(false);
    assert!(
        !squash_is_ancestor,
...
/// Property 8: Fan-out worktree ordering (via cleanup).
///
/// When a PR with multiple descendants completes (fan-out), the old worktree
...
#[test]
fn fanout_worktree_ordering_via_cleanup() {
    let (_temp_dir, config) = create_test_repo();
