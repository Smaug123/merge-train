    ...
    match spool_webhook(&repo_spool_dir, &delivery_id, &envelope) {
        Ok(_) => {
            ...
            if let Some(dispatcher) = app_state.dispatcher() {
                ...
                // Note: We intentionally do NOT trigger immediate cancellation here.
                // The stop command must be processed by the handler first to validate
                // authorization and produce a TrainStopped event. Only after successful
                // validation does the worker cancel the stack token. This prevents
                // unauthorized commenters from cancelling in-flight operations.
                //
                // The stop command will still be processed with high priority due to
                // priority queue ordering, ensuring reasonably prompt cancellation
                // after validation.
        ...
        Err(SpoolError::DuplicateDelivery(_)) => {
            ...
            if let Some(dispatcher) = app_state.dispatcher() {
                ...
                // Note: We intentionally do NOT trigger immediate cancellation here.
                // See comment above for the non-duplicate case.
