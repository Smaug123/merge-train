...
// are in step_tests.rs. Only specification-level property tests remain here.
#[cfg(test)]
#[path = "step_tests.rs"]
mod step_tests;

/// Property-based specification tests for cascade step execution.
///
/// These tests specify key invariants that must hold for correctness:
/// - Phase ordering is never violated (cascade never skips phases)
/// - Late additions don't corrupt ongoing cascades (frozen descendants)
/// - Fan-out detection works correctly
/// - Cascade completion advances or fans out appropriately
///
/// Unit tests, regression tests, and bug detection tests are in `step_tests.rs`.
#[cfg(test)]
mod property_tests {
    use super::*;
