...
impl Processor {
    ...
    fn recover_inherited(&mut self, root: PrNumber) -> Result<bool, StoreError> {
        ...
        // train's own comment: a different identity alone is no evidence —
        // an intact database restarting with its train's comment deleted
        ...
        let newer = super::adoption::choose(&trusted).filter(|(comment_id, chosen)| {
            chosen.started_at != local.started_at
                && local.status_comment_id.is_some_and(|own| *comment_id > own)
        });
        ...
        let replaced = match (newer, barred) {
            ...
            (None, Some(bar)) => {
                ...
                Some(self.adopt_replacement(root, local.clone(), Some(bar))?)
            }
            (None, None) => None,
        };
        match replaced {
