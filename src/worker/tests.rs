...
mod lost_db {
    ...
    /// - A train with NO status comment yet: the comment is the off-disk
    ///   backup, and the window between `TrainStarted` committing and its
    ///   `PostComment` landing leaves nothing to recover from. That is the
    ///   crawl's stated envelope ("a train whose comment is gone is not
    ///   resurrected"), and it is stop-shaped: the user re-issues `start`.
    fn command_loss_exemptions(
        ...
        for (root, record) in &at_loss.active_trains {
            if record.status_comment_id.is_none() {
                roots.insert(root.0);
    ...
    fn run_lost(
        ...
        // With no trace of the bot at all — not one ledger, not one status
        // comment — the crawl ONBOARDS: it reads the declarations as the
            ...
            !github.comments.values().any(|c| {
                c.author_id == TEST_BOT_ID
                    && (crate::status::parse_stack_ledger(&c.body).is_some()
                        || parse_status_comment(&c.body).is_ok())
            })
        };
    ...
    fn assert_envelope(
        ...
        for (root, record) in at_loss.active_trains.iter() {
            ...
            let reachable = !closed_unmerged_at_recovery.contains(&root.0) || {
                ...
                // root whose train it must discover — so this walk follows
                // exactly that: a PR's newest ledger, corroborated by the
                // comment it names, exactly as `crawl_events` believes it.
                let ledger_edge = |pr: u64| -> Option<u64> {
                    let (_, ledger) = github
                        .comments
                        .iter()
                        .filter(|(_, c)| c.pr.0 == pr && c.author_id == TEST_BOT_ID)
                        .filter_map(|(id, c)| {
                    ...
                    corroborated.then_some(declared.predecessor.0)
                };
                ...
                while let Some(pr) = frontier.pop() {
                    if let Some(target) = ledger_edge(pr)
                        && seen.insert(target)
                    {
            ...
            if let Some(adopted_record) = mid_phase_adoption {
                ...
                let severed = {
                    ...
                    adopted_record
                        ...
                        .any(|m| github.prs.contains_key(&m) && attested_edge(&github, m).is_none())
                };
                if severed {
                    assert_eq!(
                        squash_delta(&adopted_members),
        ...
        // corroborates it. An edit that changed the target fails that
        // corroboration; one that did not change it changes nothing.
        {
            let github = world.github.lock().unwrap();
            for (pr, cached) in &processor.state().prs {
