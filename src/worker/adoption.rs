...
/// newest INCARNATION, and within it the record supplementary recovery
/// would adopt. Incarnations are ordered by WATERMARK — the id of the
/// incarnation's FIRST status comment, which GitHub makes globally
/// monotonic and which, unlike the listing comment's own id, does not
/// move when a backup is reposted under a fresh id: a repost after a
/// deletion, the newer incarnation hidden by listing lag, would
/// otherwise outrank every incarnation for ever, and a later loss's
/// crawl would resume the train that incarnation stopped (Codex trains
/// review, P1 — reposts). A record from before the field falls back to
/// its listing id, which for a never-reposted comment IS the first
/// post's. Never by `started_at`, a wall clock that can step backwards
/// or collide across a stop-and-restart. A train's own comment is
/// UPDATED in place as it advances, and a delayed duplicate post of the
/// same incarnation can outrank the live comment while carrying an
/// older sequence, so within the incarnation (records sharing its
/// `started_at`) the record judged is the highest `recovery_seq`, ties
/// to the later comment — or the checks here run on the wrong record
/// (Codex trains review, P1 — duplicates).
pub(crate) fn choose(records: &[(CommentId, TrainRecord)]) -> Option<(CommentId, TrainRecord)> {
    let (_, newest) = records
        .iter()
        .max_by_key(|(id, r)| r.watermark.unwrap_or(*id))?;
    records
        .iter()
...
pub(crate) struct Footprint {
    ...
    /// What the record can still REACH: root, current, and the open
    /// descendant closure hanging off them. Exemption from the open-tail
    /// walk needs membership HERE — being named is not enough, since a
    /// frontier whose members merged during the gap no longer leads to a
    /// named tail (Codex trains review, P2).
    pub reachable: HashSet<PrNumber>,
...
impl Footprint {
    pub(crate) fn of(topology: &RepoState, record: &TrainRecord) -> Footprint {
        ...
        let mut reachable: HashSet<PrNumber> = closure.iter().copied().collect();
        reachable.insert(record.original_root_pr);
        reachable.insert(record.current_pr);
...
/// open) — unless an open PR still hangs behind those merges: finished
/// with a tail abandoned is not finished, and the open-tail walk aborts
/// it instead (Codex trains review, P2). A stale FAN-OUT parent is
/// completed by its children's records
...
pub(crate) fn completes(
    ...
    let all_named_merged = footprint.recorded_stack
        ...
        && open_tail_behind_history(record, footprint, present).is_none();
...
/// An OPEN PR reachable from the root only through MERGED members: the
/// train has advanced past what its record says (`1 <- 2 <- 3`, #1 and
/// #2 merged, #3 open), and walked through open PRs alone the stack ends
/// at the root. For an idle record the engine's idle path would then
/// complete the train with #3 abandoned; for a mid-phase record whose
/// frozen member merged during the gap, resuming drives the frozen
/// frontier — and completing, every named member merged, retires the
/// record — with the tail beyond those merges never prepared either way
/// (Codex trains review, P2 twice). A healthy mid-phase record is not
/// flagged: a tail reachable through OPEN members is in the record's own
/// closure. The exemption is reachability, never naming: a `known_stack`
/// member behind merged frontier members is exactly the stranded tail
...
fn open_tail_behind_history(
    ...
    while let Some(at) = queue.pop() {
        ...
        for next in present.topology.descendants.get(&at).into_iter().flatten() {
            ...
            if p.state.is_open() && !footprint.reachable.contains(next) {
                return Some(*next);
            }
...
mod tests {
    ...
    fn merged(pr: u64) -> StateEventPayload {
        StateEventPayload::PrMerged {
            pr: PrNumber(pr),
            merge_sha: Sha::parse("b".repeat(40)).unwrap(),
        }
    ...
    /// A mid-phase record is checked for an open tail behind history
    /// exactly as an idle one is: `1 <- 2 <- 3 <- 4`, the record froze
    /// `[2]`, and during the gap #2 and #3 merged. #4 is reachable from
    /// the root only through those merges — resumed, the train would
    /// drive its frozen frontier and finish with #4 never prepared
    /// (Codex trains review, P2).
    #[test]
    fn a_frozen_member_merged_behind_a_mid_phase_record_aborts() {
        let ts = crate::test_utils::test_timestamp();
        let chain = [
            opened(1, "main"),
            opened(2, "pr-1"),
        ...
        let gap: Vec<StateEventPayload> = chain
            .iter()
            .cloned()
            .chain([merged(2), merged(3)])
            .collect();
        assert_eq!(
            judge_replacement(&state(&gap), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Abort(Abort::AdvancedPastRecord(PrNumber(4)))
        );
        // The same record against the topology it froze — the stack it
        // knew, nothing merged: recovered.
        let frozen = [opened(1, "main"), opened(2, "pr-1"), declared(2, 1, 20)];
        assert_eq!(
            judge_replacement(&state(&frozen), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Recover
        );
    }

    /// The same gap on a record whose every named member merged: without
    /// the tail check it reads as FINISHED, and completing it abandons #4
    /// exactly as resuming would. Finished with an open tail behind its
    /// merges is not finished (Codex trains review, P2).
    #[test]
    fn a_record_is_not_complete_while_an_open_tail_hangs_behind_its_merges() {
        let ts = crate::test_utils::test_timestamp();
        let chain = [
            opened(1, "main"),
            opened(2, "pr-1"),
        ...
        let mut record = TrainRecord::new(PrNumber(1), ts);
        record.default_branch = "main".to_owned();
        record.cascade_phase = CascadePhase::CatchingUp {
            progress: DescendantProgress::with_known_stack(
                vec![PrNumber(2)],
                vec![PrNumber(1), PrNumber(2)],
            ),
            squash_sha: Sha::parse("c".repeat(40)).unwrap(),
        };
        let gap: Vec<StateEventPayload> = chain
            .iter()
            .cloned()
            .chain([merged(1), merged(2), merged(3)])
            .collect();
        assert_eq!(
            judge_replacement(&state(&gap), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Abort(Abort::AdvancedPastRecord(PrNumber(4)))
        );
        // With #4 merged too, nothing hangs: the record has finished.
        let done: Vec<StateEventPayload> = chain
            .iter()
            .cloned()
            .chain([merged(1), merged(2), merged(3), merged(4)])
            .collect();
        assert_eq!(
            judge_replacement(&state(&done), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Complete
        );
    }

    /// A tail the record NAMES can still be stranded: `1 <- 2 <- 3` all in
    /// `known_stack`, frozen `[2]`, and during the gap #1 and #2 merged.
    /// Naming #3 does not mean the frontier can still reach it — resumed,
    /// the engine skips merged #2 and completes with #3 never driven
    /// (Codex trains review, P2).
    #[test]
    fn a_named_tail_stranded_behind_merged_members_aborts() {
        let ts = crate::test_utils::test_timestamp();
        let chain = [
            opened(1, "main"),
            opened(2, "pr-1"),
        ...
        let gap: Vec<StateEventPayload> = chain
            .iter()
            .cloned()
            .chain([merged(1), merged(2)])
            .collect();
        assert_eq!(
            judge_replacement(&state(&gap), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Abort(Abort::AdvancedPastRecord(PrNumber(3)))
        );
        // Against the topology it froze — the tail reachable through open
        // members — the same record recovers.
        assert_eq!(
            judge_replacement(&state(&chain), PrNumber(1), &record, &HashSet::new(), None),
            Verdict::Recover
        );
    }
}
