...
>    exists for evaluates with NO delivery cause (startup, timers).
>    **Second review round (2026-09-08, the monolith re-cut into a
>    five-PR stack; GPT-5.6 on each PR):** (1) `CommentData.edited` now
>    comes from GitHub's edit history (GraphQL `lastEditedAt`) — the
>    REST `updated_at > created_at` derivation missed edits within the
>    creation second (this repo's recurring second-resolution class). (2)
>    `TrainRecord` carries provenance: `parent` (the spawning fan-out's
>    identity, root + started_at, compared by EQUALITY — the fan-out
>    replay guard and the round-8 stale-parent check no longer order
>    records by wall clock), `default_branch` (the crawl aborts an active
>    train whose default branch changed during the gap), and `watermark`
>    (the FIRST status comment's id; a recovery repost moves
>    `status_comment_id` above declarations made after the freeze).
>    (3) Receipts generalized (`status/receipt.rs`): a REJECTION receipt
>    rides on every validation rejection and late-addition answer, and the
>    crawl tombstones exactly that comment — live's decision, not the
>    crawl's re-validation against the present, decides (closes:
>    out-of-delivery-order acceptance; a comment rejected as
>    already-declared surviving the owner's later retraction; the face-(a)
>    fabrication without a heuristic). An edit that MOVES a declaration to
>    another predecessor now emits `PredecessorRemoved` first, so it gets a
>    retraction receipt. (4) The triggering delivery: an authorized
>    retraction (author deletes, or edits away) is applied by the crawl as
>    a tombstone and receipted afterwards — the deleted comment may have
>    owned the declaration, and the handler cannot see a retraction the
>    crawl has already promoted past; and a `created`/`edited` trigger
>    absent from its PR's listing is a stale redelivery of a since-deleted
>    comment, closed unhandled. The example-test harness now mirrors comment
>    webhooks into the fake (a delivery describes a comment that exists).
>    (5) The harness itself was reworked to the reviews: regimes are
>    forced BY CONSTRUCTION (each property runs once per regime with the
>    regime baked into the strategy — a train under every gap move kind,
>    the wake-up kind rotating — so failures shrink and replay within
>    their regime) and coverage floors are asserted after every run;
>    late declarations are delivered by the batch driver at saga
>    boundaries so they genuinely race the train; the gap keeps only
>    each comment's FINAL fate; redeliveries replay ORIGINAL payloads
>    (deleted/edited comments included); the store-vs-reality check
>    compares merge SHAs. The forced regimes immediately found a P1 the
>    random harness had missed: a frozen member UNSTACKED during the gap
>    (its declaration deleted — live aborts on that removal) was driven
>    by the recovered train; the crawl now aborts when any fetched frozen
>    member has no recorded predecessor, and the extension check covers
>    every declaration INTO the known stack, not only the walk from the
>    root. A `created` redelivery for a comment edited since is closed as
>    stale like one for a deleted comment. The former face-(a) allowance
>    is gone from the differential oracle (rejection receipts made it
>    unnecessary). (6) Terminal status-comment updates are OWED until they
>    land (`owed_status_syncs`, store schema v5; the executor reports
>    best-effort outcomes; retried at the stall cadence): a terminal train's
>    comment is the crawl's only source, and one left saying "active" by
>    an outage resurrected a stopped train after a later DB loss (monolith
>    review, P1). Residual: a DB loss during the same outage — bounded by
>    the outage, where before it was unbounded. (7) Second-round crawl
>    fixes: a stale trigger's close commits ATOMICALLY with the crawl's
>    events (a crash between would let the retry bypass the check); the
>    freshness check covers `edited` payloads (body must match the listed
>    body) and PR webhooks (an old unmerged `closed` for a PR the crawl
>    found open would abort the recovered train); an UNAUTHORIZED
>    triggering edit shapes nothing (the pipeline rejects it next);
>    fan-out evidence is any trusted record on a member naming this parent
>    incarnation, not only the member's latest record; a synthesized
>    completion is a real `TrainCompleted` after the adoption, so the store
>    owes the stale comment its final word; omitted (`0`) author ids never
>    match. Restatement ownership moves only FORWARD in comment id (the
>    receipt tombstone's invariant; also closes the round-15 residual).
>    (8) Third-round fixes: the triggering retraction tombstones only where
>    the retracted comment could have OWNED the declaration (a surviving
>    declaration above its id proves otherwise; the bodyless-deletion
>    residual — deleting an unrelated later comment drops the edge — is
>    stop-shaped); `opened`/`reopened` triggers must agree with the crawled
>    head, base and draft, not merely "still open", since the handler writes
>    those payload fields over the crawl's; the fixpoint's individual
>    `GetPr` fetches are capped (200) with the remainder treated as
>    unfetchable, and tombstoned declarations no longer drive it at all —
>    unbounded, a junk history could exhaust the API quota and pause the
>    repo's queue indefinitely. Terminal syncs are keyed by COMMENT, not
>    root (a root can retire twice under two comments), and outcomes match
>    by comment, since an observation boundary appends another train's
>    terminal cleanup to whichever batch is in flight.
>    (9) Fourth-round fixes: the extension check no longer infers
>    application order from comment ids — `DescendantProgress` now records
>    the WHOLE stack the train knew at each freeze (`known_stack`), so a
>    declaration created before the status comment but DELIVERED after it
>    is correctly a late extension; the id watermark survives only as the
>    fallback for records written before the field. A triggering comment a
>    receipt already killed is reported by the crawl and its delivery
>    closed (handled fresh it could be accepted against today's topology),
>    and a stale trigger's crawl is RECOMPUTED without it, so its
>    retraction tombstones nothing and no receipt is posted. Terminal
>    syncs are keyed by train INCARNATION with a nullable comment id: a
>    `PostComment` that landed before the process died leaves a live
>    comment the store has no id for, and the retry resolves it by the
>    incarnation embedded in the bot's comments.
>    (10) Fifth/sixth-round fixes: an owed sync is SCHEDULED where it is
>    created (the cleanup carries an `UpdateComment` only when the record
>    knew its comment id), the retry prefers the stored id (a mangled body
>    there still needs the rewrite) and falls back to incarnation matching,
>    and its store errors propagate rather than stranding the obligation.
>    The crawl: a triggering comment ABSENT from the listing is released
>    and re-crawled ONCE before being believed (GitHub is not
>    read-after-write consistent, and closing on first absence silently
>    lost valid commands); `known_stack` always contains the root and
>    current PR so "empty" unambiguously means a legacy record; the
>    all-members-merged staleness check reads the whole known stack (a
>    three-deep train whose first two levels merged was being completed
>    with its tail still open); incarnations are chosen by the monotonic
>    comment id rather than wall-clock `started_at`; `opened`/`reopened`
>    triggers must agree on the head REF too; and the per-PR comment crawl
>    is capped (1000) so a very large repository cannot spend its whole
>    rate-limit window and never bootstrap. Polling: a DUE poll runs at a
>    fixed point in the worker turn rather than only when idle.
>    Events-table compaction — **landed
...
## Amendment (2026-09-08): the crawl reads, it does not derive

The first-contact crawl rebuilt the predecessor graph by replaying the
users' declaration comments through the live path's validation. Ten rounds
of review found ten more ways for that reconstruction to disagree with the
decision it was reconstructing, because the inputs it needed — the order
the deliveries arrived in, who edited a comment (the API reports only its
original author), the state of every PR at the time — no longer exist. The
receipts of the previous amendment were an attempt to make the derivation's
NEGATIVE decisions durable; they did not converge either, because the
positive ones were still being re-derived.

**The decision is written down.** When the bot records a predecessor — or
removes one — it rewrites that PR's *stack ledger* comment (`status::ledger`):
bot-authored, machine-readable, sitting on the PR it names, carrying the
predecessor, the comment that declared it, the store sequence number the
write states, and the highest declaration comment on that PR whose fate the
bot has settled.

The write is OWED, like a terminal status update: the obligation is a dirty
set written in the topology event's own transaction, it names only the PR
(what to write is read from the state at write time, so repeated changes
coalesce into one correct write), and clearing it is conditional on the
sequence number, so a change made while a write was in flight is not
discharged by it. Discharging probes the PR's comments first, which adopts
the orphan a crash between `PostComment` and `StackLedgerPosted` leaves
rather than posting a second record. A ledger comment somebody deletes
re-dirties itself from the webhook.

**The crawl then obeys three rules.**

1. *The ledger grants.* An edge exists because a ledger says the bot
   recorded one — never because a comment can be read as declaring one.
2. *The owning comment may revoke.* If the comment a ledger names is gone,
   or no longer declares that predecessor, the edge is dropped. Two durable
...
repository with no trace of the bot — not one ledger, not one status
comment — has never had a decision made about it, so its declarations are
read as the live path would read them and recorded. No train can exist
there to endanger. Accepted residual: a repository whose every bot comment
was deleted looks new, and its declarations are adopted afresh.

This deleted the receipts module and about 250 lines of `crawl_events`. The
guarantee the conformance harness now asserts is one sentence: **the
recovered topology is exactly what the ledgers attest** — and the ledger is
one write behind the state, in both directions, which is the documented
residual.
