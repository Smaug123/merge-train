...
1. Query the **direct** descendants from the `descendants` index (PRs that directly declare #N as their predecessor), filtering to only **open** PRs. Transitive descendants (children of children) are NOT included in the frozen set — they will be handled when their direct predecessor becomes the current PR. This is correct because preparation merges #N's head into descendants, and transitive descendants have a different predecessor (their base is not #N's branch). Example: if #A has descendants #B (closed) and #D (open), and #B has descendant #C (open), only #D is frozen for #A. #C is not reachable because #B is closed (blocks traversal), and even if #B were open, #C belongs in #B's frozen set, not #A's, because #C's predecessor is #B.
