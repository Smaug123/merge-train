...
1. Original train (`original_root_pr=123`) completes after #123 merges
2. Two NEW trains are created: one with `original_root_pr=124`, one with `original_root_pr=125`
3. Each gets its own worktree, its own entry in `active_trains`
4. The old train (keyed by 123) is removed from `active_trains`

**Status comment handoff**: When fan-out completes, update #123's status to "Train completed. Descendants #124, #125 are now independent trains", then post initial status comments on #124 and #125.

**Fan-out recovery**: If crash occurs mid-fan-out, scan for status comments on merged PRs with `state = "running"`. If found, query descendants and create trains for those already retargeted to main; mark others as `needs_manual_review`.

### Per-stack worktrees

Each stack gets its own isolated git worktree (`stack-<original_root_pr>/`), enabling:
...
**Worktree lifecycle**:
1. **Creation**: On `@merge-train start` for PR #123, create `stack-123/`
2. **During cascade**: Continue using `stack-123/` even as current_pr advances
3. **Completion/Stop**: Remove worktree
4. **Fan-out**: Remove old worktree BEFORE creating new worktrees for each descendant

**Critical ordering for fan-out**: The old worktree must be removed before creating new worktrees to ensure clean state and prevent accumulation.

**Stale worktree cleanup**: On startup, remove worktrees older than 24 hours not associated with active trains.

### Key type summaries

See `src/types/train.rs` for full definitions. Key points:
...
Additional tests:
- `multi_file_content_preserved_after_cascade` — extends Property 1 to multi-file scenarios
- `multiple_intervening_commits_preserved` — extends Property 2 to pre/post-squash commits
- `late_additions_dont_corrupt_cascade` (`src/cascade/step.rs:1842`) — verifies frozen set invariant through full cascade
- `worktree_cleanup_leaves_no_orphans` — verifies cleanup removes exactly non-active worktrees
- `catch_up_detects_conflicts_with_main` — verifies conflict detection during catch-up
- `naive_ordering_loses_late_commits` — regression test demonstrating why $SQUASH_SHA^ is required
